	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navigate with ~y~W,A,S,D~s~ and verify with ~y~SPACE~s~ for code block to the left.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Use ~y~Arrow Keys~s~ and ~y~ENTER~s~ for code block to the right")
			else
